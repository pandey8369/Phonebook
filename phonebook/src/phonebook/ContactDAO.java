/*
 * The file mainly represents the Data Access Object (DAO) class for managing contact in phone book.
 */
package phonebook;

import phonebook.ConnectionFactory;
import phonebook.ContactDTO;
...
public class ContactDAO {
    ...
    public void addFunction(ContactDTO ContactDTO) {
        try {
            String query = "INSERT INTO Contacts VALUES(null,?,?,?,?)"; //Inserting the value in the database
    ...
    public  void editContactDAO(ContactDTO ContactDTO) {
        try {
            String query = "UPDATE Contacts SET lastname=?,location=?,phone=? WHERE firstName=?"; //Updating the records of database
            prepStatement = conn.prepareStatement(query);
    ...
    public void deleteContactDAO(String custCode) {
        try {
            String query = "DELETE FROM Contacts WHERE firstName='" +custCode+ "'"; //Deleting the record 
 ...
 // Method to retrieve data set to be displayed
    public ResultSet getQueryResult() {
        try {
            String query = "SELECT firstName,lastname,location,phone FROM Contacts"; //use search a particular record
            resultSet = statement.executeQuery(query);
        } catch (SQLException e) {
            e.printStackTrace();
        }
